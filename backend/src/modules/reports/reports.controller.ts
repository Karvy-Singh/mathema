...
export class ReportsController {
  ...
  nextFocus(@CurrentUser('id') userId: string) { return this.service.nextFocus(userId); }

  @Get('weekly/achievements')
  achievements(@CurrentUser('id') userId: string) { return this.service.achievements(userId); }

  // 메타인지 캘리브레이션 — confidence vs 실제 정답률
  @Get('calibration')
  calibration(@CurrentUser('id') userId: string) { return this.service.calibration(userId); }
