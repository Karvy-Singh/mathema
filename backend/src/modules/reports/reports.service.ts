...
export class ReportsService {
  ...
  async nextFocus(userId: string) {
    ...
    const priorities = ['최우선', '높음', '중간'];
    const impacts = ['+8점 예상', '+5점 예상', '+3점 예상'];
    return masteries.map((m, i) => ({
      unit: m.unit.name,
      area: { '미적분 II': '회전체의 부피', '확률·통계': '정규분포 표준화', '기하·벡터': '공간좌표 시각화' }[m.unit.name] ?? '약점 보강',
      priority: priorities[i] ?? '중간',
      color: palette[i] ?? '#A16207',
      impact: impacts[i] ?? '+1점 예상',
    }));
  }
   ...
   */
  async calibration(userId: string) {
    ...
    if (attempts.length === 0) {
      return {
        ...
        attemptCount: 0,
        buckets: [],
        insight: '자신감 입력이 필요합니다. 답 제출 시 슬라이더로 확신도를 표시하면 캘리브레이션이 분석돼요.',
      };
    }
    ...
    if (Math.abs(avgGap) < 5) {
      insight = `잘 보정되어 있어요 (Brier ${brier.toFixed(3)}). 자신감과 실제 정답률이 거의 일치합니다.`;
    } else if (avgGap > 0) {
      insight = `자신감이 실제 정답률보다 평균 ${Math.round(avgGap)}%p 높아요 (overconfidence). 풀이 직후 검산 단계를 추가하면 정확도와 자신감의 격차가 줄어듭니다.`;
    } else {
      insight = `자신감이 실제 정답률보다 평균 ${Math.round(-avgGap)}%p 낮아요 (underconfidence). 알고 있는 내용에 더 확신을 가져도 됩니다 — 실전에서 시간 낭비를 줄일 수 있어요.`;
    }
